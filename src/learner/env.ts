/** Env isolation for the learner subprocess: the child env is built by allowlist from a stripped
 * base, since any inherited ANTHROPIC_ or CLAUDE_CODE_ variable could reroute or misbill it. */

export type LearnerTrigger = "bootstrap" | "hook" | "manual";

export interface LearnerEnvOptions {
...
/** Prefixes whose variables must never reach the learner subprocess. */
...
/** Build the learner subprocess env: the caller's env minus every Anthropic/Claude-Code variable,
 * plus the gateway wiring. */
export function buildLearnerEnv(options: LearnerEnvOptions): NodeJS.ProcessEnv {
