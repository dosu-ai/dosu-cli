...
- **`@dosu/api-types` and `@trpc/*` move in lockstep.** `@dosu/api-types` ships type-only declarations generated against a specific `@trpc/server` (see its `dependencies`). When bumping `@dosu/api-types`, bump `@trpc/client` and `@trpc/server` to that **same** version in the *same commit* — a mismatch makes the whole tRPC router resolve to `never`, flooding `tsc` with phantom "does not exist on type 'never'" errors that mask the real drift. `@dosu/api-types` is first-party (generated from our own router), so it's listed in `minimumReleaseAgeExcludes` and can be adopted immediately. The scheduled `api-types-drift` workflow type-checks against `@dosu/api-types@latest` (with the matching `@trpc/*`) to flag when the pinned types have fallen behind the live backend.
