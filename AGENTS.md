...
- **`src/shipper/`** — Opt-in transcript shipping to the Dosu memory ingest API (`POST /v1/memory/ingest/async`, fire-and-forget). The sync pipeline's `ship` step: honors the `/dosu-incognito` per-session opt-out, normalizes raw session logs to Letta trajectory-v1 via `@letta-ai/trajectory`, redacts every outgoing string with `redactSecrets`, and records the accepted `task_id`/`session_url` in the ship watermark state (independent of the studying watermark, same `knowledge-sync.json`). Gated by the `ship_transcripts` flag — default OFF, toggled with `dosu knowledge transcripts enable|disable|status`. Long-term this path replaces local studying (`mine`); both run side by side today.
